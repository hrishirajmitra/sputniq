...
def deploy_app(config, extract_dir: Path):
    try:
        ...
        for agent in config.agents:
            ...
            shutil.copytree(Path(__file__).parent.parent, service_dir / "sputniq", dirs_exist_ok=True, ignore=shutil.ignore_patterns("__pycache__", "*.pyc"))
...
def teardown_app(run_id: str) -> int:
    ...
    try:
        ...
        containers = client.containers.list(
            all=True,
            filters={"label": f"sputniq.run_id={run_id}"},
        )
        removed_count = 0
        for c in containers:
